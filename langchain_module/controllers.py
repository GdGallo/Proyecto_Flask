...
def chat_with_memory():
    ...
    user_input = data.get("input")
    if not session_id or not user_input or not user_input.strip():
        return jsonify({"error": 'Los campos "input" y "session_id" son obligatorios y "input" no puede estar vacío.'}), 400

    try:
        # Recuperar memoria de la sesión
